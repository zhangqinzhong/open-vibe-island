...
这是一个开源的 [Vibe Island](https://vibeisland.app/) 替代品，面向 macOS 上的重度 code agent 用户。目前支持 **Claude Code**、**Codex**、**Cursor**、**OpenCode**、**Qoder**、**Qwen Code**、**Factory** 和 **CodeBuddy**，终端集成覆盖 **Terminal.app**、**Ghostty**、**cmux**、**Kaku**、**WezTerm**、**iTerm2** 和 **Zellij**，工作区级跳转支持 **VS Code**、**Cursor**、**Windsurf**、**Trae** 和 **JetBrains 全家桶**（IDEA、WebStorm、PyCharm、GoLand、CLion、RubyMine、PhpStorm、Rider、RustRover），并对 Warp 提供降级检测支持。
...
本应用可能会为你的 Claude Code、Codex 或 Cursor 安装 hooks，因此你可能会在这些会话中看到 hook 相关输出。
...
| **VS Code** | 工作区跳转 | 通过 `code` CLI 激活工作区 |
| **VS Code Insiders** | 工作区跳转 | 通过 `code-insiders` CLI 激活工作区 |
| **Cursor** | 工作区跳转 | 通过 `cursor` CLI 激活工作区 |
| **Windsurf** | 工作区跳转 | 通过 `windsurf` CLI 激活工作区 |
| **Trae** | 工作区跳转 | 通过 `trae` CLI 激活工作区 |
| **JetBrains 全家桶** | 工作区跳转 | 通过 IDE CLI 激活项目（IDEA、WebStorm、PyCharm、GoLand、CLion、RubyMine、PhpStorm、Rider、RustRover） |
| **Warp** | 规划中 | 仅降级检测 |
