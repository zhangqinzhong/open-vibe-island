...
这是一个开源的 [Vibe Island](https://vibeisland.app/) 替代品，面向 macOS 上的重度 code agent 用户。目前支持 **Claude Code**、**Codex**、**Cursor**、**OpenCode**、**Qoder**、**Factory** 和 **CodeBuddy**，终端集成覆盖 **Terminal.app**、**Ghostty**、**cmux**、**Kaku**、**WezTerm**、**iTerm2** 和 **Zellij**，并对 Warp 提供降级检测支持。
